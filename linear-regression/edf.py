...
class GatingParams(ParameterPackage):
    def __init__(self,nInputs):
        X = Xavier(nInputs)
        self.A = Parameter(np.random.uniform(-X,X,(nInputs,)))

class Gating(CompNode):
    def __init__(self,w,x):
        CompNodes.append(self)
        self.x = x
        self.w = w
        self.sigmoid = Sigmoid(self.w.A)

    def forward(self):
        self.sigmoid.forward()
        self.value = self.sigmoid.value * self.x.value

    def backward(self):
        self.x.addgrad(self.grad * self.sigmoid.value)
        self.w.A.addgrad(self.grad * self.x.value)
